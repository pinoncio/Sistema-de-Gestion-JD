...
test("submits the form and calls onSubmit prop", async () => {
  ...
  fireEvent.change(screen.getByLabelText(/Nombre del Gasto/i), {
    target: { value: "Test gasto" },
  });
  fireEvent.change(screen.getByLabelText(/Detalle/i), {
    target: { value: "Detalle test" },
  });
  fireEvent.change(screen.getByLabelText(/Fecha de Compra/i), {
  ...
  fireEvent.change(screen.getByLabelText(/Pago Neto/i), {
    target: { value: "10000" },
  });
