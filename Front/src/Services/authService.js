...
const loginUser = async (rut, contrasenia) => {
  try {
    ...
    console.log("Datos enviados al servidor:", { rut_usuario: rut, contrasenia_usuario: contrasenia });
    ...
    if (err.response) {
      ...
      switch (err.response.status) {
        ...
        case 403:
          throw new Error("La cuenta está deshabilitada temporalmente. Contacta al administrador.");
        case 404:
          throw new Error("El RUT ingresado no está registrado.");
        default:
          // Si es otro código de error, puedes manejarlo como un error genérico
          throw new Error(err.response.data?.msg || "Error en el inicio de sesión");
      }
    }
